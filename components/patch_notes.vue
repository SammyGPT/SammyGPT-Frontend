<template>
    <div v-if="!seenPatchNotes" id="patch-notes" class="dark:text-secondary text-primary absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 shadow-lg rounded-sm p-5 bg-zinc-700">
        ...
        <Button text="Close" :selected="true" @clicked="closePatchNotes" class=" rounded-md dark:bg-secondary bg-primary p-2 px-5 dark:text-black text-white absolute left-[75%] bottom-6"/>
    </div>
</template>
